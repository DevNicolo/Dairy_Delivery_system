...
class ResPartner(models.Model):
    ...
    #Probably useless funcions

    @api.onchange("agent")
    def _onchange_agent_clear_agents(self):
        """Clear agent_ids when agent is set to True."""
        if self.agent:
            self.agent_ids = False

    @api.constrains("agent", "agent_ids")
    def _check_agent_cannot_have_agents(self):
        """Prevent an agent from having reference agents."""
        for record in self:
            if record.agent and record.agent_ids:
                raise models.ValidationError(
                    "Un agente non può avere agenti di riferimento. "
                    "Quando il partner è marcato come agente, il campo "
                    "'Agenti' deve essere vuoto."
                )
